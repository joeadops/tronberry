...
static void DrawFrame(FrameCanvas* canvas, const uint8_t* frame_data, int width, int height) {
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            int index = (y * width + x) * 4; // RGBA
            canvas->SetPixel(x, y, frame_data[index], frame_data[index + 1], frame_data[index + 2]);
        }
    }
}
...
static void DisplayAnimation(RGBMatrix* matrix, FrameCanvas *&canvas, WebPAnimDecoder* anim_decoder, int width, int height, int brightness, int dwell_secs) {
    ...
    while (std::chrono::steady_clock::now() - start_time < std::chrono::seconds(dwell_secs)) {
...
static int usage(const char *progname, const char *msg = NULL) {
    ...
    fprintf(stderr, "Fetch images over HTTP and display on RGB-Matrix\n");
...
int main(int argc, char* argv[]) {
    ...
    if (scheme_end == std::string::npos) {
        std::cerr << "Invalid URL: Missing scheme (http:// or https://)" << std::endl;
        return 1;
    }
    size_t path_start = url.find('/', scheme_end + 3); // Find the start of the path after the scheme
    httplib::Client client(url.substr(0, path_start != std::string::npos ? path_start : url.length())); // Extract base URL
    std::string path = path_start != std::string::npos ? url.substr(path_start) : "/"; // Extract path or default to "/"
    ...
    std::thread fetch_thread([&]() {
        ...
        while (running) {
            ...
            if (!res || res->status != 200) {
                std::cerr << "Failed to fetch image from URL" << std::endl;
                int wait_time = std::min(1 << retry_count, 60); // Exponential backoff with max wait time of 60 seconds
                wait_time = std::max(wait_time, 1); // Ensure at least 1 second wait time
            ...
            response.brightness = std::stoi(res->get_header_value("Tronbyt-Brightness"));
            response.dwell_secs = std::stoi(res->get_header_value("Tronbyt-Dwell-Secs"));

            {
                std::unique_lock<std::mutex> lock(queue_mutex);
                queue_not_full.wait(lock, [&]() { return response_queue.size() < max_queue_size; });
    ...
    while (running) {
        ...
        WebPData webp_data = {reinterpret_cast<const uint8_t*>(response.data.data()), response.data.size()};
        WebPAnimDecoderOptions anim_options;
        WebPAnimDecoderOptionsInit(&anim_options);
        WebPAnimDecoder* anim_decoder = WebPAnimDecoderNew(&webp_data, &anim_options);
